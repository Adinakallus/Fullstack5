...
const Todos = (juser) => {
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      try {
        await fetchObj.fetchData(`todos?userId=${userRef.current.id}`);
  ...
  const handleAddTodo = async () => {
    ...
    const newTodoItem = {
      userId: parseInt(userRef.current.id),
  ...
  return (
    <div className="todos-container">
      <div className="todos-header">
        <h2>{userRef.current.username}'s Todos</h2>
